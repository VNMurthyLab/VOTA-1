...
class VOTASniffMeasure(Measurement):
    ...
    def run_trial(self,lick):
        ...
        '''
            ...
            if self.trial_tick>(self.settings.lick_interval.value()*1000):
...
#                     water_tick=0
                ...
                self.buffer[i,2]=bool(int(4-self.buffer[i,2])) #convert lick sensor into 0(no lick) and 1(lick)
