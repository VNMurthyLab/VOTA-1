...
class VOTASolCalibrationMeasure(Measurement):
    ...
    def setup(self):
        ...
        self.ui_filename = sibling_path(__file__, "sol_calibration_plot.ui")
        ...
        self.settings.New('selected_sol',dtype = int, initial = 0, vmin = 0, vmax = 7, ro = False)
        self.settings.New('forward',dtype = bool, initial = True, ro = False)
        self.settings.New('calibrate_on',dtype = bool, initial = False, ro = False)
    ...
    def run(self):
        ...
        self.buffer = np.zeros((10000, 14), dtype=float)
        ...
        try:
            ...
            while not self.interrupt_measurement_called:
                ...
                if self.settings.calibrate_on.value():
                    ...
                    sol_chan = self.settings.selected_sol.value()
                    self.buffer[i, 6 + sol_chan] = current_value
                    self.arduino_sol.load(self.buffer[i,6:14])
                '''
