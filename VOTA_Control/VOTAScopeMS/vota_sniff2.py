...
class VOTASniffMeasure(Measurement):
    ...
    def update_display(self):
        ...
        self.success_plot.setData(self.stat_buffer[0:ntrial,1])
        self.failure_plot.setData(self.stat_buffer[0:ntrial,2])
        self.no_action_plot.setData(self.stat_buffer[0:ntrial,3])
    ...
    def calc_stats(self):
        if self.settings.num_of_trial.value()>0:
            ...
            self.stat_buffer[self.settings.num_of_trial.value(),0]=self.time_tick
            self.stat_buffer[self.settings.num_of_trial.value(),1]=self.settings.success_percent.value()
    ...
    def gen_stim(self,go_chance=0.8,ng_stim_chance=1):
        ...
        if dice>go_chance:
            ...
            if dice<ng_stim_chance:
                if not self.settings.ng_odor.value()==0:
            ...
            for i in range(repeats):
                if not self.settings.go_odor.value()==0:
    ...
    def run_trial(self):
        ...
        if self.settings.punishment.value():
            ...
            if self.trial_tick>(self.settings.task_duration.value()*1000):
                ...
                '''
                reward_onset_time=self.settings.reward_onset.value()
